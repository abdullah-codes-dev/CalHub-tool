...
import Head from "next/head"; // <-- Added for SEO / canonical
	...
	return (
		<>
			<Head>
				...
				/>
				<link rel="canonical" href="https://www.jazzcasher.site/tools/percentage-calculator" />
			</Head>
				...
				<div className='max-w-4xl mx-auto'>
					...
					<Tabs defaultValue='basic' className='w-full'>
						...
						<TabsContent value='increase'>
							<Card>
								<CardHeader>
									<CardTitle> Calculate Percentage Increase/Decrease </CardTitle>
									<CardDescription> Add or subtract a percentage from a number </CardDescription>
								</CardHeader>
								<CardContent className='space-y-4'>
									<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
										<div>
									...
									{increaseResult !== null && (
										<div className='text-center p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg'>
											...
											<div className='text-sm text-gray-600'>
												{increaseValue}{" "}
												{Number.parseFloat(increasePercent) >= 0 ? "increased" : "decreased"}{" "}
												by {Math.abs(Number.parseFloat(increasePercent))}% = {increaseResult.toFixed(2)}
											</div>
										</div>
									)}
								</CardContent>
						...
						<TabsContent value='change'>
							<Card>
								<CardHeader>
									...
									<CardDescription> Find the percentage change between two values </CardDescription>
								</CardHeader>
								<CardContent className='space-y-4'>
									<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
										<div>
											<Label htmlFor='changeOld'> Original Value </Label>
											<Input
												id='changeOld'
												type='number'
												placeholder='e.g., 50'
									...
									{changeResult !== null && (
										<div className='text-center p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg'>
											...
											<div className='text-sm text-gray-600'>
												Change from {changeOld} to {changeNew} = {changeResult.toFixed(2)}%{" "}
